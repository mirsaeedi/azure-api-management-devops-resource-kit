		...
		public void Map(IMapperConfigurationExpression cfg)
		{
			cfg.CreateMap<ApiDeploymentDefinition, ApiProperties>()
				...
				.ForMember(dst => dst.Format, opt => opt.MapFrom(
					src => src.GraphQlEndpoint != null ? GraphQlFormat.GraphQlLink : new OpenApiSpecReader(src.OpenApiSpec).GetOpenApiFormat().Result))
				.ForMember(dst => dst.Value, opt => opt.MapFrom(
					src => src.GraphQlEndpoint != null ? src.GraphQlEndpoint : new OpenApiSpecReader(src.OpenApiSpec).GetValue().Result))
